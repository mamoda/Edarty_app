// Dashboard.tsx - النسخة النهائية الكاملة
        ...
        <div className="flex items-start justify-between">
          <div className="space-y-2">
            ...
            <p className="text-3xl font-bold text-gray-900 tracking-tight">
              {prefix}
              {safeValue.toLocaleString(language === "ar" ? "ar-EG" : "en-US", {
...
const DataAccuracyIndicator: React.FC<{
  validation: StatisticsValidation;
  onRefresh: () => void;
}> = ({ validation, onRefresh }) => {
  const { language } = useLanguage();
        ...
        <div className="flex items-start gap-3">
          ...
          <div className="flex-1">
            ...
            <p className="text-xs text-amber-700 mt-1">
              ...
              {language === "ar" ? "مشكلة في البيانات" : "data issues found"}
            </p>
            <button
              onClick={onRefresh}
...
const UpdateStatusBar: React.FC<{ lastUpdated: number }> = ({
  lastUpdated,
}) => {
  const { language } = useLanguage();
...
export default function Dashboard() {
          ...
          if (teacher.status === "active") {
            ...
            if (validateAmount(salary)) {
              ...
              validationErrors.push(
                `Invalid salary for teacher ${teacher.id}: ${teacher.salary}`,
              );
            }
          }
        });
  ...
  // حساب جميع التrends مرة واحدة لتجنب التكرار
  ...
  const activeStudentsTrend = calculateTrend(
    ...
    previousStats.activeStudents,
  );
  // في قسم تعريف المتغيرات، عدل هذا السطر

  const teachersTrend = calculateTrend(
    stats.totalTeachers ?? 0,
    previousStats.totalTeachers ?? 0,
  );
  ...
  const profitTrend = calculateTrend(stats.netProfit, previousStats.netProfit);
          ...
          <div className="flex gap-6">
            ...
            <main className="flex-1 min-w-0">
              ...
              {currentView === "students" && (
                <StudentsManager onUpdate={() => loadStatistics(false)} />
              )}
              {currentView === "teachers" && (
                <TeachersManager onUpdate={() => loadStatistics(false)} />
              )}
              {currentView === "fees" && (
                <FeesManager onUpdate={() => loadStatistics(false)} />
              )}
              {currentView === "expenses" && (
                <ExpensesManager onUpdate={() => loadStatistics(false)} />
              )}

...
const loadData = async () => {
      ...
      <head>
        ...
        <style>
          ...
          * { margin: 0; padding: 0; box-sizing: border-box; }
          ...
          .school-name { font-size: 28px; font-weight: 800; background: ${isRefund ? 'linear-gradient(135deg, #dc2626, #b91c1c)' : 'linear-gradient(135deg, #059669, #047857)'}; -webkit-background-clip: text; -webkit-text-fill-color: transparent; background-clip: text; margin-bottom: 5px; }
          .school-info { font-size: 12px; color: #6b7280; margin-top: 5px; line-height: 1.6; }
          .receipt-title { font-size: 18px; color: #6b7280; margin-top: 5px; font-weight: 600; }
          .receipt-number { background: ${isRefund ? '#fef2f2' : '#f0fdf4'}; padding: 15px; border-radius: 12px; text-align: center; margin-bottom: 20px; border: 1px solid ${isRefund ? '#fee2e2' : '#dcfce7'}; }
          .receipt-number .value { font-size: 20px; font-weight: 800; ${isRefund ? 'color: #dc2626;' : 'color: #059669;'} font-family: monospace; letter-spacing: 1px; }
          .details { margin-bottom: 20px; background: #f9fafb; padding: 15px; border-radius: 12px; }
          .detail-row { display: flex; justify-content: space-between; padding: 10px 0; border-bottom: 1px solid #f3f4f6; }
          .amount-section { background: ${isRefund ? 'linear-gradient(135deg, #fef2f2, #fee2e2)' : 'linear-gradient(135deg, #f0fdf4, #dcfce7)'}; padding: 20px; border-radius: 12px; text-align: center; margin: 20px 0; border: 1px solid ${isRefund ? '#fecaca' : '#bbf7d0'}; }
          .amount-section .value { font-size: 42px; font-weight: 800; ${isRefund ? 'color: #dc2626;' : 'color: #059669;'} line-height: 1.2; }
      ...
      <body>
        <div class="receipt">
          <div class="header">
            ...
            <div class="school-info">📧 ${currentReceipt.school_email || schoolEmail}</div>
          ...
          <div class="details">
            ...
            <div class="detail-row"><span>📅 تاريخ ${isRefund ? 'الاسترداد' : 'الدفع'}:</span><span>${formatDate(currentReceipt.payment_date)}</span></div>
            <div class="detail-row"><span>💳 طريقة ${isRefund ? 'الاسترداد' : 'الدفع'}:</span><span>${paymentMethodLabel}</span></div>
          </div>
  ...
  const handleDelete = async (id: string) => {
    if (!hasPermission('delete_fees')) {
      alert("ليس لديك صلاحية لحذف العمليات المالية");
      return;
    }
  ...
  const handleEdit = (fee: Fee) => {
    if (!hasPermission('edit_fees')) {
      alert("ليس لديك صلاحية لتعديل العمليات المالية");
      return;
    }
      ...
      <body>
        <div class="statement">
          <div class="header">
            ...
            <div style="font-size: 12px; color: #6b7280;">نظام إدارة المصاريف الدراسية</div>
          </div>
          <div class="balance-info">
            <div class="balance-card"><div>إجمالي المدفوعات</div><div class="balance-value">${balances?.total_paid.toFixed(2)} ج.م</div></div>
            <div class="balance-card"><div>صافي المدفوعات</div><div class="balance-value">${balances?.net_paid.toFixed(2)} ج.م</div></div>
            <div class="balance-card"><div>الرصيد الحالي</div><div class="balance-value" style="color: ${balances && balances.balance >= 0 ? "#059669" : "#dc2626"}">${balances?.balance.toFixed(2)} ج.م</div></div>
          </div>
          <table class="transactions-table">
            <thead><tr><th>التاريخ</th><th>البيان</th><th>المبلغ</th><th>الرصيد</th></tr></thead>
            <tbody>
              ${transactions.map(t => `<tr><td>${formatDate(t.date)}</td><td>${t.description}</td><td class="${t.type === "deposit" || t.type === "installment" ? "deposit" : "withdrawal"}">${t.type === "deposit" || t.type === "installment" ? "+" : "-"}${t.amount.toFixed(2)}</td><td>${t.balance_after.toFixed(2)}</td></tr>`).join("")}
            </tbody>
          </table>
          <div class="footer"><p>نظام إدارتي - إدارة المصاريف الدراسية</p></div>
        </div>
  ...
  const canAddFee = hasPermission('add_fees') || hasPermission('edit_fees');
